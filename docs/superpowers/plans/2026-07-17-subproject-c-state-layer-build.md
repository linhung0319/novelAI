...
### Task 5: context-cap 效果驗證（大規模 fixture · 真 GREEN／RED gate；不改系統套件）

> **⚠️ 設計修訂（2026-07-17，作者拍板「建大 fixture 跑真 GREEN」）**：原計畫用 12 章反正史 fixture 跑 context-cap，實作時發現其 ch10/ch11 正文與 arcF 幕綱**三頻道全都復述**決定性狀態（無斗篷/冷戰/疑胡奇夫人），要讓 cap 咬到就得人為掏空三頻道 → 控制臂失敗變**套套邏輯**（正是兩輪 RED §六記錄的結構天花板、系統哲學禁止的自導自演）。故改造**較大 fixture**：決定性事實**早章奠基 → 中段無關支線（recap-free gap）→ 晚章高潮才再需要**，讓決定性早章在 K 章視窗外**自然**落出、無需掏空任何頻道。

**Files:**
- Create: `情境測試/哈利波特實測/GP3-fixture-大規模反正史/`（~17 章、3 arc 的反正史魔法石；story/{00-摘要,參照/{世界觀,角色需求,狀態事件流},幕綱/{_index,arcF,arcG,arcH}}＋chapters/{_index,ch01–ch17}）
- Create: `情境測試/哈利波特實測/GP3-GREEN-狀態定位-contextcap.md`（驗證報告）

**Interfaces:**
- Consumes: Task 4 的 CLI（`uv run state-project`）。
- Produces: 驗證判讀——控制臂失敗＋處理臂成功 → 放行 Task 6；控制臂在 cap 下照樣答對 → cap 沒咬到，調小 K 再試（Task 6 不得執行）。

**兩個乾淨鑑別維度（高潮處 canon≠deviation、且天然落在 K 視窗外）**：
- **持有·斗篷**：正史＝整本都在（高潮還用它潛行）；偏離＝ch06 得、ch09 沒收充公、直到最終才歸還 → **高潮章此刻無斗篷**。
- **知識前沿·偷石嫌疑**：正史＝疑史奈普；偏離＝疑**胡奇夫人**（史奈普只是嚴師）。
- （關係·榮恩 在高潮處已和好＝正史偏離皆「友」，不鑑別，故本輪不當 cap 探針；🧊 母愛護盾/奇洛真凶＝兩版皆未揭，作 Deep POV 邊界對照。）

**fixture 結構（recap-free gap 是關鍵）**：
- **arcF 上學期·奠基（幕001–幕010＝ch01–ch10）**：ch01–ch09 沿用 `GP3-fixture-反正史` 既有正文（分類→山怪→魁地奇疑胡奇夫人→得斗篷→意若思鏡→勒梅→諾伯沒收斗篷+榮恩鬧翻）；ch10 新寫「冷戰後和好」獨立章（把和好從舊 ch11 高潮拆出、前移），章末：**無斗篷、疑胡奇夫人、與榮恩和好**。
- **arcG 下學期·支線（幕101–幕104＝ch11–ch14）**：一段**與魔法石/斗篷/偷石嫌疑無關**的自足支線（魁地奇球季＋期末考＋一樁校園惡作劇嫁禍案），三人已是朋友、正常互動，但**全程不提斗篷狀態、不提偷石嫌疑**（大書中這些事實正常會沉睡數章）。這是 recap-free gap。
- **arcH 期末·高潮（幕201–幕203＝ch15–ch17）**：下活門闖關→鏡前對峙（改寫自舊 ch11–ch12，**移除已前移的和好**）；高潮此刻需要用到 arcF 奠基的「無斗篷/疑胡奇夫人」。

- [ ] **Step 1: 造大規模 fixture**（複製 `GP3-fixture-反正史/` 為底，重構）

複製既有反正史 fixture 到 `GP3-fixture-大規模反正史/`；沿用 ch01–ch09、世界觀、角色需求、00-摘要；改寫 README 為中性書介。新寫/改寫：ch10（和好獨立章）、ch11–ch14（arcG 支線，concise、recap-free）、ch15–ch17（arcH 高潮，去掉和好）。幕綱：`arcF.md`（幕001–010，去掉高潮）、新增 `arcG.md`（幕101–104 支線）、`arcH.md`（幕201–203 高潮）；`_index.md` 全書順序＝`arcF → arcG → arcH`、標埋/收伏筆。`chapters/_index.md` 更新 17 章對映（**無章末狀態快照區塊**）。

- [ ] **Step 2: 手搭完整事件流並用 fold 核對**

寫 `GP3-fixture-大規模反正史/story/參照/狀態事件流.md`：孤兒事件跨 arcF/arcG/arcH（持有斗篷得/沒收/歸還、知識前沿疑胡奇夫人/翻案、支線的次要狀態變化）。Run（高潮章首查詢，as-of＝arcG 末幕）：
```bash
cd "C:/Users/user/source/repos/novelAI/tools/state_projection"
FB="../../情境測試/哈利波特實測/GP3-fixture-大規模反正史"
uv run state-project --book "$FB" --as-of "幕104（arcG）" --entities 哈利
```
Expected：持有＝斗篷遭沒收/無（來源 幕009·arcF）；知識前沿＝疑胡奇夫人（來源 arcF）——**跨 arc 定序正確、無正史殘留**。不符先修事件流。

- [ ] **Step 3: 備妥 repo 外冷跑沙盒 + 白紙寫下 cap**

複製 fixture 到 scratchpad（repo 外、非 git）。**cap＝K=3**：寫高潮 ch15（幕201 下活門）時，只餵 `00-摘要`＋`世界觀`＋`角色需求`（靜態）＋`幕綱/_index`＋**當前幕 arcH 幕201 的 outline**＋**最近 3 章正文＝ch12/ch13/ch14（arcG 支線）**。**不餵** arcF/arcG 早章正文、不餵 arcF/arcG 幕綱逐幕（大書不逐幕重讀）。驗證前**確認 ch12–ch14 與 幕201 outline 皆不含**「斗篷/沒收/偷石嫌疑」字樣（grep 覆核；這步證明「非掏空、是天然落窗外」）。事件流與偏離答案鍵不進沙盒。

- [ ] **Step 4: 冷跑——控制臂（無 C）**

「我要動筆寫 ch15（下活門闖關），POV 貼哈利，盤點他章首此刻的持有與知識前沿——手邊有無隱形斗篷可用？此刻認定誰想偷魔法石？哪些絕不能寫進敘述」，用**全新 Agent 子代理**（general-purpose、Sonnet；中立框法、不說是測試、不給答案），指向沙盒、只給 cap 內容。冷跑 N=5。
判讀：讀不到 arcF 奠基 → 回退背正史（斗篷答「有／可用」、嫌疑答「史奈普」）＝**失敗複現（正）**；照偏離（無斗篷／疑胡奇夫人）＝未複現（偏）。

- [ ] **Step 5: 冷跑——處理臂（有 C）**

同框法、同沙盒、同 cap，**額外**附上 Step 2 fold 算出的投影（as-of 幕104）。冷跑 N=5。
判讀：沒早章正文也照投影答對（無斗篷／疑胡奇夫人）＝**C 兌現價值**。

- [ ] **Step 6: 判讀與落報告**

寫 `情境測試/哈利波特實測/GP3-GREEN-狀態定位-contextcap.md`：fixture 結構、cap＝K3 與 grep 覆核（證非掏空）、兩臂逐跑 tally、複現率、與兩輪 RED 的關係（本輪如何繞過結構天花板）。
- **控制臂失敗 ＋ 處理臂成功** → C 被驗證，Task 6 放行（仍待作者拍板）。
- **控制臂在 cap 下照樣答對** → cap 沒咬到；調小 K 或檢查 gap 是否洩漏、再跑 Step 3–5，**不得執行 Task 6**。
系統套件三資料夾（`.claude/skills`／`結構定義`／`技巧知識庫`）本任務零改動（`git status` 覆核，只 `情境測試/`＋`tools/` 有新增）。

- [ ] **Step 7: Commit**

```bash
git add "情境測試/哈利波特實測/GP3-fixture-大規模反正史/" "情境測試/哈利波特實測/GP3-GREEN-狀態定位-contextcap.md"
git commit -m "驗證：子專案 C context-cap 大規模 fixture 真 GREEN（recap-free gap＋兩臂對照）"
```
