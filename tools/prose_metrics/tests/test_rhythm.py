...
def _stat(group, cjk=10_000, dashes=0, jolts=0, echoes=0, short=100):
    from prose_metrics.rhythm import RhythmStat

    return RhythmStat(
        group=group, chapters=5, cjk=cjk, paras=300, dashes=dashes, jolts=jolts,
        echoes=echoes, short_paras=short, sent_lens=(), clause_lens=(), commas=0, periods=0,
    )
...
def test_one_sentence_per_paragraph_is_the_corpus_norm():
    """句/段：六本 ≈1（段落就是句子），生成書 1.7–2.1——顛簸指數的機制解釋。

    生成版的形狀是「多句擠成的密集長段 ＋ 夾在中間的斷片段」，長短在段落級擺盪；
    語料通篇同一顆粒度，沒有可擺盪的空間。本輪從語料反推得到，診斷03 未載。
    """
    good = [s for b in _SIX for s in _corpus_stats(b)]
    gen = _book_stats(_KNOWN_BAD) + _book_stats(_CONTROL)
    assert max(s.sent_per_para for s in good) < 1.35
    assert min(s.sent_per_para for s in gen) > 1.35
...
def test_short_is_not_the_crime():
    ...
    assert detect_rhythm([ji]) == []  # 三項簽名仍全清
...
def test_thresholds_sit_in_the_gap():
    """門檻不是拍腦袋：三項都落在 known-good max 與生成 min 之間。"""
    good = [s for b in _SIX for s in _corpus_stats(b)]
