...
四項簽名，本檔實測全語料（前三項每千漢字；回聲報佔 ≤12字非對白段的比例）：

                            破折號密度   顛簸指數   回聲佔比      句/段   分段
    known-good 六本語料       0.00–0.38  0.08–0.23  0.0–0.6%   0.96–1.25   8
    known-bad  生成一世之尊    6.42–9.76  1.87–3.51  0.0–9.1%   1.41–2.11  11 arc
    對照       生成芯片巫師    8.61–9.81  1.66–2.40  0.0–6.5%   1.89–2.33   3 arc
...
≥40字句字數佔比僅 17%，它比生成版更短更碎，而它在 known-good 名單裡——它的短行
每一行都有主謂、都在推進事件，且通篇維持同一速度。判準因此不是「句子要長」，
是這四條：**不要無資訊的重拍、不要高頻煞車、不要把逗號升級成句號、段落是句子的容器**。
`test_rhythm.py` 把極道釘死成回歸案例，就是為了防止日後有人把本檔誤讀成「寫長一點」。

已測過但**不成立**：長句佔比與句長離散度
--------------------------------------
第二輪診斷問過「該長的地方沒放長」能不能做成第五項門檻——**不能，而且不是差一點，
是完全重疊**。全語料實測（2026-07-25）：

                            known-good 六本   生成一世之尊   生成芯片巫師
    ≥40字句**字數**佔比        17.4–54.0%       27.2%        40.1%
    ≥2×中位長句字數佔比        23.0–44.3%       42.7%        44.2%
    ≥1.5×中位長句字數佔比      44.2–59.0%       60.5%        61.6%   ← 生成版更高
    句長變異係數 (σ/μ)         0.58–0.92        0.77         0.78    ← 正中央
    p90/中位                   2.00–3.14        2.50         2.53    ← 正中央

**「生成版缺句長變異」這個直覺是錯的。** 這四個量都得做成**下限**（「至少要有這麼多
長句／這麼大變異」），而生成書在**每一項上都不低於 known-good 的下緣**——任何抓得到
生成書的下限，都會先誤傷《極道天魔》。六本自己就從 17% 到 54%，長句佔比是**文類自由**
的量。「該長的地方要長」因此是**書級**問題——對照本書參照語料（`--corpus`）或
`可用句式` 宣告，不是普世地板。
`test_sentence_length_dispersion_does_not_separate` 把這張表釘死，防止重試。

**真正的抓手是第四項簽名 句/段**，而它與長句佔比是同一件事的兩面：

    語料 一世之尊：非對白段中位 34 字 = 句長中位 27 × 句/段 1.22
    生成 一世之尊：非對白段中位 21 字 = 句長中位 16 × 句/段 1.69

**生成版的段落裡本來就有足夠的材料，只是在句號那一級被切開了。** 把一段內的兩三句
用逗號串回一句，句長與逗號:句號自動回到語料區間——不必另外下「多寫長句」的指令
（那類讀感指令實測會被在最粗的一級實現，正是本病徵的成因）。
"""
...
# 第四項（第二輪追加）。它與前三項有兩點不同，兩點都寫在 finding 文字裡，別拿掉：
#
# ① **餘裕窄得多**。分段層級 known-good 六本 0.96–1.25、生成兩本 1.41–2.33，門檻
#    1.35 距上緣僅 1.08×、距下緣僅 1.04×（對照破折號的 1.3×／12.8×）。零重疊仍然
#    成立，但它比前三項更容易被邊界個案擦到——**這是刻意接受的取捨**：它是三項簽名
#    之外唯一還有零重疊的量，而缺口「該長的地方沒放長」只能靠它帶動（見檔頭）。
# ② **六本語料全是網文**，而「一句一段」是網文的排版慣例。文學向書系的密集長段可能
#    在此誤報。三項簽名可以說「跨文類收斂」，這一項只能說「跨網文文類收斂」。
SPP_CAP = 1.35  # 句/段
...
class RhythmStat:
    ...
    def sent_per_para(self) -> float:
        """句/段。段落是不是「一個句子的容器」——第四項門檻，也是顛簸指數的機制解釋。

        六本語料 0.96–1.25（一句一段是常態），兩本生成書 1.41–2.33，無重疊。生成版的
        形狀因此是：**多句擠成的密集長段 ＋ 夾在中間的斷片段**，長短在段落級高頻擺盪；
        語料則通篇同一顆粒度，沒有可擺盪的空間。

        第二輪從輔助量升為門檻，理由是它同時扛下了「該長的地方沒放長」——段落裡的
        材料本來就夠，只是在句號那一級被切開（見檔頭的兩行算式）。餘裕與文類的兩點
        保留意見寫在 `SPP_CAP` 上。
        """
    ...
    # ---- 輔助量（供對照 `可用句式` 宣告，非門檻）
    #
    # ⚠️ 別想把 `long_sent_share` 升成門檻——已測過，兩組完全重疊，見檔頭
    # 「已測過但不成立」。長句佔比是文類自由的量，只能對照本書語料或宣告看。
    @property
...
def detect_rhythm(
    ...
    **advisory**：命中＝候選，交作者判斷，不是 pass/fail。刻意的全短風格
    ...
    for s in stats:
        ...
        if s.sent_per_para > spp_cap:
            findings.append(
                Finding(
                    group=s.group,
                    metric="句/段",
                    detail=(
                        f"{s.sent_per_para:.2f}（上限 {spp_cap}，語料 0.96–1.25）"
                        "——段落是句子的容器：多句擠成密集長段，再夾入斷片當重拍，"
                        "長短就在段落級擺盪。改法是把段內的兩三句用逗號**串成一句**，"
                        "不是拆成兩段（拆會推高顛簸）。"
                        "⚠️ 這一項餘裕比前三項窄，且六本語料全是網文——"
                        "文學向書系的密集長段可能是刻意的，交作者判斷"
                    ),
                )
            )
    return findings
